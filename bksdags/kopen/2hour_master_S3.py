...
with DAG(
    'Kopen_Master_2Hour_db2S3minio_dag',
    ...
    t1 = PythonOperator(
        ...
        op_kwargs={'From_Table': "unit_retain", 'To_Table': "kp_machine", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': ""}
    )

    t2 = PythonOperator(
        task_id='prepare_kopen_machine',
        provide_context=True,
    ...
    t3 = PythonOperator(
        task_id='copy_machine_to_s3_data_lake',
        provide_context=True,
        python_callable= common.copy_to_minio,
        op_kwargs={'From_Table': "unit_retain", 'To_Table': "kp_machine", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': "", 'Last_Days': 1, 'FY':''}
    )
    t3.set_upstream(t2)

    t4 = PythonOperator(
        task_id='copy_machine_to_s3sl_data_lake',
        provide_context=True,
        python_callable= common.copy_to_minio_sl,
        op_kwargs={'From_Table': "unit_retain", 'To_Table': "kp_machine", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': "", 'Last_Days': 2, 'FY':''}
    )
    t4.set_upstream(t3)

    t5 = PythonOperator(
        trigger_rule=TriggerRule.ALL_DONE,
        task_id='etl_kopen_machine_data_lake',
        provide_context=True,
    ...
    t6 = PythonOperator(
        ...
        provide_context=True,
        python_callable=EL_process,
        op_kwargs={'From_Table': "customer", 'To_Table': "kp_customer", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': ""}
    )
    t6.set_upstream(t5)

    t7 = PythonOperator(
        task_id='prepare_kopen_customer',
        provide_context=True,
        python_callable= PP_process,
        op_kwargs={'From_Table': "customer", 'To_Table': "kp_customer", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': ""}
    )
    ...
    t8 = PythonOperator(
        task_id='copy_customer_to_s3_data_lake',
        provide_context=True,
        python_callable= common.copy_to_minio,
        op_kwargs={'From_Table': "customer", 'To_Table': "kp_customer", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': "", 'Last_Days': 1, 'FY':''}
    )
    t8.set_upstream(t7)

    t9 = PythonOperator(
        task_id='copy_customer_to_s3sl_data_lake',
        provide_context=True,
        python_callable= common.copy_to_minio_sl,
        op_kwargs={'From_Table': "customer", 'To_Table': "kp_customer", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': "", 'Last_Days': 2, 'FY':''}
    )
    t9.set_upstream(t8)

    t10 = PythonOperator(
        trigger_rule=TriggerRule.ALL_DONE,
        task_id='etl_kopen_customer_data_lake',
        provide_context=True,
        python_callable= ETL_process,
        op_kwargs={'From_Table': "customer", 'To_Table': "kp_customer", 'Chunk_Size': 50000, 'Key': 'item_id', 'Condition': ""}
    )
