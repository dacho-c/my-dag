def sql_ET_cust_rank(st):
    return """select A.PROVINCE, A.cus_province__c as province_sf, A.CUS_ID, A.CUS_NAME, A.CUS_ENAME, A.BRANCH, A.REGION as BRANCH_REGION, A.major_brand__c as major_brand, A.customer_type__c as TYPE_SF, A.region_by_user__c as SALES_REGION, A.sales_name as SALES_PERSON, A.BUSINESS, A.MC as TOTAL_MC, A.KOM as KOMTRAX, A.NKOM as NON_KOMTRAX,
...
COALESCE(part.AMT14,0) as amt14, COALESCE(sA.amt14,0) as samt14,
COALESCE(part.TAMT,0) as tamt, COALESCE(sA.tamt,0) as stamt, (COALESCE(part.TAMT,0) + COALESCE(sA.tamt,0)) as ttamt,
mc14, mc15, mc16, mc17, mc18, mc19, mctotal, sB.customer_type__c, sB.region_by_user__c, case when sB.sales_name = 'Admin Mintel' then null else sB.sales_name end sales_name, sB.cus_province__c, sB.major_brand__c 
from kp_customer C 
...
-- MC last Delivery
left outer join (select A.UR_CUS, sum(A.MC14) as mc14, sum(A.MC15) as mc15, sum(A.MC16) as mc16, sum(A.MC17) as mc17, sum(A.MC18) as mc18, sum(A.MC19) as mc19, sum(A.MC) as mctotal
from (select A.UR_CUS, case when Y = (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 6 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 end) then MC else 0 end mc14,
...
EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 6 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 end)::varchar(255) || '-04-01')::date) A) A group by A.UR_CUS) M on (C.CUS_ID = M.UR_CUS)
-- End MC
-- Part Amount
left outer join (select PIH_INVOICED_CUS_ID, sum(AMOUNT14) as amt14, sum(AMOUNT15) as amt15, sum(AMOUNT16) as amt16,
sum(AMOUNT17) as amt17, sum(AMOUNT18) as amt18, sum(AMOUNT19) as amt19, sum(amount) as Tamt
from (select PIH_INVOICED_CUS_ID,
case when Y = (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 6 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 end) then AMOUNT else 0 end amount14,
...
where PIH_ACCOUNT_MONTH >= (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 6 else  EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 end) || '04' and PIH_ACCOUNT_MONTH <= to_char(current_date - INTERVAL '1 day','YYYYMM') 
...
-- End Part
-- Service Amount
left outer join (select PIH_INVOICED_CUS_ID, sum(AMOUNT14) as amt14, sum(AMOUNT15) as amt15, sum(AMOUNT16) as amt16, sum(AMOUNT17) as amt17, sum(AMOUNT18) as amt18, sum(AMOUNT19) as amt19, sum(AMOUNT) as tamt
from (select PIH_INVOICED_CUS_ID,
case when Y = (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 6 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 end) then AMOUNT else 0 end amount14,
case when Y = (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 5 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 4 end) then AMOUNT else 0 end amount15,
case when Y = (case when EXTRACT(MONTH FROM (current_date - INTERVAL '1 day')) < 4 then EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 4 else EXTRACT(YEAR FROM (current_date - INTERVAL '1 day')) - 3 end) then AMOUNT else 0 end amount16,
...
-- End Service Amount
left outer join (select sa.eg_kopen_id__c, sa.cus_province__c, sa.customer_type__c, sa.major_brand__c, sa.region_by_user__c , su."name" as sales_name, (su.firstname_th__c || ' ' || su.lastname_th__c) as name_th
