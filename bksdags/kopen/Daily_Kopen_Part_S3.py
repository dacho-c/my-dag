...
with DAG(
    ...
    t3 = PythonOperator(
        ...
        op_kwargs={'From_Table': "PRODUCT", 'To_Table': "kp_part", 'Chunk_Size': 50000, 'Key': 'pro_komcode', 'Condition': "", 'Last_Days': 1, 'FY':''}
    )
    t3.set_upstream(t2)

    t4 = PythonOperator(
        task_id='copy_part_to_s3sl_data_lake',
        provide_context=True,
        python_callable= common.copy_to_minio_sl,
        op_kwargs={'From_Table': "PRODUCT", 'To_Table': "kp_part", 'Chunk_Size': 50000, 'Key': 'pro_komcode', 'Condition': "", 'Last_Days': 2, 'FY':''}
    )
    t4.set_upstream(t3)
