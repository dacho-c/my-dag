...
def sql_part_sale_detail(fy, fy1):
    ...
    A.PSD_GUIDELINES, A.PSD_LP_AMOUNT, A.PSD_PROMOTION_CODE, B.PSH_ACCOUNT_MONTH from PART_SALE_DETAIL A
    join (select PSH_ACCOUNT_MONTH, PSH_TICKET_ID FROM PART_SALE_HEAD where left(PSH_ACCOUNT_MONTH,4) in ('%s','%s')) B on(A.PSD_TICKET_ID = B.PSH_TICKET_ID)""" % (fy,fy1)
